...
curl -sSL https://sc.repo.cloud.strettch.com/metrics/install.sh | sudo bash
```
</CodeGroup>
...
```
curl -sSL https://sc.repo.cloud.strettch.com/metrics/install.sh -o /tmp/install.sh
```
...
```
If the service is not running or these commands return no output, contact [Strettch Cloud support](mailto:cloud@strettch.com) for help troubleshooting the installation.